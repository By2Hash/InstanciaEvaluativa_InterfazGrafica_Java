...
static void registrarAsistencia() {
    ...
    System.out.print("  ¿Estudiante presente? (S/N): ");
    String resp = scanner.nextLine();
    boolean presente = resp.equalsIgnoreCase("S");

    // Ejecuta la lógica en el objeto
    ins.registrarAsistencia(presente);

    // Obtiene el porcentaje para las alertas
    double porcentaje = ins.getPorcentajeAsistencia();
    
    System.out.println("----------------------------------------");
    System.out.printf("  Asistencia registrada con éxito.%n");
    System.out.printf("  Porcentaje actual: %.1f%%%n", porcentaje);

    // Lógica de alertas (Zona de riesgo y Regularidad)
    if (porcentaje < 75) {
        System.out.println("  [!!!] ALERTA CRÍTICA: Asistencia bajo el 75%.");
        System.out.println("        ESTADO: LIBRE (Pierde regularidad).");
    } 
    else if (porcentaje < 80) {
        System.out.println("  [!] ADVERTENCIA: Asistencia bajo el 80%.");
        System.out.println("      ESTADO: ZONA DE RIESGO.");
    }
    System.out.println("----------------------------------------");
}
